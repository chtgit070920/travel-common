...
//      final String countSql = "select count(1) " + removeSelect(removeOrders(sql));
        ...
        try {
        	...
        	if (conn == null){
        		...
        		if(ds instanceof AtomikosDataSourceBean){//atomikos 
        			AtomikosDataSourceBean adsb=(AtomikosDataSourceBean)ds;
        			conn=adsb.getXaDataSource().getXAConnection().getConnection();
        		}else{
